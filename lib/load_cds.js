...
function load_cds_data(loaded) {
  $.getJSON(cds_byloc_url, (response, status, xhr) => {
    ...
    var names = _.concat('Country State County FIPS Population'.split(' '),
       _.range(ds, df).map(i => date_from_num(i)));
    var cds = {};
    var table = [names]
    for (var field of 'cases deaths confirmed recovered tested'.split(' ')) {
      _.forEach(response, (record) => {
        var country = COUNTRIES[record.country] || null;
        ...
        _.forEach(record.dates, (r, d) => {
          var dat = field == 'cases' ? r.cases || r.confirmed || r.active :
                    r[field];
          dayrow[num_from_date(d) - ds] = dat;
        });
        table.push(_.concat(
            [country, state, county, fips, record.population], dayrow));
      })
      cds[field] = table;
    }
    loaded(cds);
