    ...
    Chartist.plugins.legend = function (options) {
        ...
        return function legend(chart) {
            ...
            function createNameElement(i, legendText, classNamesViable) {
                ...
                // Chartist hard-codes a cycle of 26 colors, so match it.
                li.classList.add('ct-series-' + (i % 26));
            ...
            function applyLegendStateToLegend(legends) {
                for (var i = 0; i < legends.length; i++) {
                    legendElement.childNodes[i].classList.toggle('inactive', !legends[i].active);
                }
            }
                    ...
                    if (!legend.active) {
                        ...
                        if (!options.removeAll) {
                            if (activeCount == 0) {
                                ...
                                // invert them instead:
                                for (var i = 0; i < legends.length; i++) {
                                    legends[i].active = (i != legendIndex);
                                }
                                applyLegendStateToLegend(legends);
                            } else if (activeCount == legends.length - 1) {
                                // Inverse: do not disable just one
                                for (var i = 0; i < legends.length; i++) {
                                    legends[i].active = !legends[i].active;
                                }
                                applyLegendStateToLegend(legends);
                            }
                        }
            ...
            var initState = options.legendState;
            legendNames.forEach(function (legend, i) {
                ...
                legends.push({
                    text: legendText,
                    series: legendSeries,
                    active: (!initState.length || initState.indexOf(legendText) >= 0)
                });
            });
            if (!options.removeAll) {
                // Special case: if all legends are disabled,
                // then ignore initialization and enable everything.
                if (legends.filter(function (legend) {
                        return legend.active; }).length == 0) {
                    legends.forEach(function (legend, i) {
                        legend.active = true;
                    });
                }
            }
            // To get the proper colors, we need to arrange for series class
            // names before the first call to applyLegendStateToChart.
            ...
            function createHandler(data) { 
                // For print support: 'create'
                chart.off('created', createHandler);
                appendLegendToDOM(legendElement);
            }
            chart.on('created', createHandler)
