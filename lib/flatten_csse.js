...
function flatten_all(feed) {
  ...
  for (var seriesname in feed) {
    ...
    [ts_names, all_totals, all_locinfo] = rollup_locinfo_ts(
        feed[seriesname], keys_for_locality);
  ...
  merged_locinfo = _.mapValues(merged_locinfo, v => _.mapKeys(v, (vv, k) =>
      k == 'Admin2' ? 'county' :
      k.match(/state/i) ? 'state' :
      k.match(/country/i) ? 'country' :
      k.toLowerCase()));
  merged_locinfo = _.mapValues(merged_locinfo, v => {
    v.level = v.county ? 'county' : v.state ? 'state' : 'country';
    return v;
  });
  return _.mapValues(records, (v, loc) => {
    ...
    for (var i = num_from_date(v.first_date); ; ++i) {
      ...
      series.push(_.defaults({daynum: i}, v[d], props));
    }
    return series;
  });
}
